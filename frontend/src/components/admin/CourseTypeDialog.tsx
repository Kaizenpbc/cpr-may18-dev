...
function CourseTypeDialog({ open, onClose, onSave, courseType }) {
    ...
    if (open) {
      if (isEditMode && courseType) {
        setFormData({
          ...
          duration:
            courseType.duration !== null ? String(courseType.duration) : '',
          maxstudents:
            courseType.maxstudents !== null
              ? String(courseType.maxstudents)
              : '',
        });
      } else {
  ...
  const handleChange = event => {
    ...
    if (
      (name === 'duration' || name === 'maxstudents') &&
      value &&
      !/^[0-9]*$/.test(value)
    ) {
  ...
  const handleSave = async () => {
    ...
    if (!formData.coursetypename)
      newFieldErrors.coursetypename = 'Course Type Name required';
    ...
    if (!formData.maxstudents)
      newFieldErrors.maxstudents = 'Max Students required';
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth='sm' fullWidth>
      <DialogTitle>
        {isEditMode ? 'Edit Course Type' : 'Add New Course Type'}
      </DialogTitle>
      <DialogContent>
        {error && (
