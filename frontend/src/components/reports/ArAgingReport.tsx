...
const ArAgingReport = () => {
  ...
  if (!reportData || !reportData.buckets)
    return <Typography sx={{ m: 2 }}>No aging data available.</Typography>;
            ...
            {Object.entries(bucketMap).map(([key, bucketInfo]) => (
              <React.Fragment key={key}>
                <TableRow
                  hover
                  onClick={() =>
                    bucketInfo.data.invoices.length > 0 &&
                    handleBucketClick(key)
                  } // Clickable if invoices exist
                  sx={{
                    cursor:
                      bucketInfo.data.invoices.length > 0
                        ? 'pointer'
                ...
                >
                  ...
                  <TableCell align='right'>
                    {formatCurrency(bucketInfo.data.total)}
                  </TableCell>
                </TableRow>
                ...
                {expandedBucketKey === key && (
                  <TableRow>
                    ...
                    <TableCell colSpan={2} sx={{ p: 0, border: 0 }}>
                      <BucketDetailTable
                        invoices={bucketInfo.data.invoices}
                        bucketTitle={bucketInfo.label}
                      />
                    </TableCell>
                  </TableRow>
                )}
