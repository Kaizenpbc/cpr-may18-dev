      ...
      try {
        ...
        const formattedData = data.map
          ? data.map(item => ({
              ...
              endingBalance:
                item.balanceBroughtForward +
                item.totalInvoiced -
                item.totalPaidInMonth,
            }))
          : [];
  ...
  return (
    <Paper sx={{ p: 2, mt: 2 }}>
      ...
      {isLoading ? (
        ...
        <Typography sx={{ m: 2 }}>
          No revenue data found for {selectedYear}.
        </Typography>
      ) : (
        <Box mt={2}>
          ...
          <TableContainer component={Paper} variant='outlined'>
            <Table size='small'>
              ...
              <TableBody>
                {reportData.map(row => {
                  const totalDue =
                    row.balanceBroughtForward + row.totalInvoiced;
                  return (
                    <TableRow key={row.month} hover>
                      <TableCell>{row.month}</TableCell>
                      <TableCell align='right'>
                        {formatCurrency(row.balanceBroughtForward)}
                      </TableCell>
                      <TableCell align='right'>
                      ...
                      <TableCell align='right'>
                        {formatCurrency(row.totalPaidInMonth)}
                      </TableCell>
                      <TableCell align='right'>
                ...
                >
                  ...
                  <TableCell align='right'>
                    {formatCurrency(
                      reportData.reduce(
                        (sum, row) => sum + row.totalPaidInMonth,
                        0
                      )
                    )}
                  </TableCell>
