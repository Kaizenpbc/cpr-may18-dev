...
const PaidVendorInvoices: React.FC = () => {
  ...
  const fetchPaidInvoices = async () => {
    try {
      ...
      const paidInvoices = (response.data || []).filter((invoice: any) => 
        invoice.status === 'paid'
      );
  ...
  const { isConnected } = useVendorInvoiceUpdates({
    onStatusUpdate: (update) => {
      console.log('🔄 Real-time status update received in paid invoices:', update);
    ...
    onNotesUpdate: (update) => {
      console.log('📝 Real-time notes update received in paid invoices:', update);
    },
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={6}>
                    ...
                    <Box display="flex" alignItems="center" mb={1}>
                      ...
                      <Typography variant="body2">
                        <strong>Email:</strong> {selectedInvoice.vendor_email}
                      </Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <Box display="flex" alignItems="center" mb={1}>
                      ...
                      <Typography variant="body2">
                        <strong>Payment Method:</strong> {selectedInvoice.vendor_payment_method?.replace('_', ' ').toUpperCase() || 'CHECK'}
                      </Typography>
                    </Box>
                  </Grid>
                </Grid>
