...
const InstructorWorkloadSummaryTable = ({ workloads = [] }: { workloads?: any[] }) => {
  ...
  return (
    <TableContainer component={Paper} sx={{ mt: 1, mb: 3 }}>
      ...
      <Typography variant='h6' sx={{ p: 2, pb: 1 }}>
        Instructor Workload (Current Month)
      </Typography>
      <Table
        stickyHeader
        size='small'
        aria-label='instructor workload summary table'
      >
        <TableHead>
          <TableRow>
          ...
          {sortedWorkloads.map(wl => (
            <TableRow key={wl.instructorId} hover>
              ...
              <TableCell align='center'>
                {wl.scheduledUpcomingThisMonth}
              </TableCell>
              <TableCell align='center' sx={{ fontWeight: 'bold' }}>
                {wl.totalWorkloadThisMonth}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
