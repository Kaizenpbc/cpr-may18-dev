            ...
            <Table stickyHeader size="small" aria-label="invoice history table">
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell align="center" sx={{ fontWeight: 'bold' }}>Posted</TableCell>
                ...
                <TableBody>
                    {invoices.map((invoice, index) => (
                        ...
                        >
                            <TableCell>
                                <strong>{invoice.invoicenumber || invoice.invoice_number}</strong>
                            </TableCell> 
                            ...
                            <TableCell>
                                <MuiLink component={RouterLink} to={`/accounting/organizations/${invoice.organizationid || invoice.organization_id}`} underline="hover">
                                    {invoice.organizationname || invoice.organization_name || '-'}
                                </MuiLink>
                            </TableCell>
                            <TableCell>{invoice.coursenumber || invoice.course_number || '-'}</TableCell>
                            <TableCell>{invoice.course_type_name || '-'}</TableCell>
                            <TableCell>{invoice.location || '-'}</TableCell>
                            <TableCell>{formatDate(invoice.date_completed || invoice.course_date)}</TableCell>
                            <TableCell align="center">{invoice.students_billed || '-'}</TableCell>
                            <TableCell align="right">
                                <strong>{formatCurrency(invoice.amount)}</strong>
                            </TableCell>
                            <TableCell align="right">{formatCurrency(invoice.paidtodate || invoice.paid_to_date || 0)}</TableCell>
                            <TableCell align="right">
                                <strong>{formatCurrency(invoice.balancedue || invoice.balance_due || invoice.amount)}</strong>
                            </TableCell>
                            <TableCell align="center">
                                <Chip 
                                    label={invoice.paymentstatus || invoice.payment_status || invoice.status || 'Unknown'} 
                                    color={getStatusChipColor(invoice.paymentstatus || invoice.payment_status || invoice.status)} 
                                    size="small"
                                />
                            </TableCell>
                            <TableCell>{invoice.agingbucket || invoice.aging_bucket || '-'}</TableCell> 
                            <TableCell align="center">
                                <Chip 
                            ...
                            <TableCell align="center">
                                <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', justifyContent: 'center' }}>
                                    <Tooltip title="Preview Invoice">
                                        <IconButton 
                                            size="small" 
                                            onClick={() => handlePreview(invoice.invoiceid || invoice.invoice_id)}
                                            color="primary"
                                        >
                                    ...
                                    <Tooltip title="Download PDF">
                                        <IconButton 
                                            size="small" 
                                            onClick={() => handleDownloadPDF(invoice.invoiceid || invoice.invoice_id, invoice.invoicenumber || invoice.invoice_number)}
                                            color="secondary"
                                        >
