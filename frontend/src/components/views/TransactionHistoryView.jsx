                        ...
                        <Paper sx={{ p: 2, mt: 3 }}>
                            ...
                            <Grid container spacing={3}>
                                <Grid item xs={6} sm={3}>
                                    ...
                                    <Typography variant="h6">{filteredInvoices.length}</Typography>
                                </Grid>
                                <Grid item xs={6} sm={3}>
                                    <Typography variant="body2" color="text.secondary">Total Amount</Typography>
                                    <Typography variant="h6">
                                        ${filteredInvoices.reduce((sum, inv) => sum + parseFloat(inv.amount || 0), 0).toFixed(2)}
                                    </Typography>
                                </Grid>
                                <Grid item xs={6} sm={3}>
                                    <Typography variant="body2" color="text.secondary">Total Paid</Typography>
                                    <Typography variant="h6" color="success.main">
                                        ${filteredInvoices.reduce((sum, inv) => sum + parseFloat(inv.paidtodate || inv.paid_to_date || 0), 0).toFixed(2)}
                                    </Typography>
                                </Grid>
                                <Grid item xs={6} sm={3}>
                                    <Typography variant="body2" color="text.secondary">Total Outstanding</Typography>
                                    <Typography variant="h6" color="error.main">
                                        ${filteredInvoices.reduce((sum, inv) => sum + parseFloat(inv.balancedue || inv.balance_due || inv.amount || 0), 0).toFixed(2)}
                                    </Typography>
                                </Grid>
                            </Grid>
                        </Paper>
