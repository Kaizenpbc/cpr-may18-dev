...
export const useNetworkAwareAPI = () => {
  ...
  const makeRequest = useCallback(
    async <T = any>(options: APIRequestOptions): Promise<APIResponse<T>> => {
      ...
      if (skipIfSlowConnection && isSlowConnection) {
        logger.warn(
          `[NetworkAwareAPI] Skipping request due to slow connection: ${method} ${url}`
        );
        return {
      ...
      if (!canMakeRequest() && !queueWhenOffline) {
        return {
          error: isOnline
            ? 'Connection too slow for this request'
      ...
      if (!isOnline) {
        try {
          ...
          if (cached) {
            ...
            if (age < 60 * 60 * 1000) {
              logger.info(
                `[NetworkAwareAPI] Returning cached data for: ${url}`
              );
              return {
                data,
                fromCache: true,
  ...
  const shouldMakeRequest = useCallback(
    (options?: { skipIfSlowConnection?: boolean; requireOnline?: boolean }) => {
      const { skipIfSlowConnection = false, requireOnline = false } =
        options || {};
  ...
  const getRequestRecommendations = useCallback(() => {
    ...
    if (!isOnline) {
      recommendations.push(
        "You're offline. Requests will be queued and synced when connection is restored."
      );
    } else if (isSlowConnection) {
      recommendations.push(
        'Slow connection detected. Consider deferring large uploads or downloads.'
      );
    }
    ...
    switch (connectionQuality) {
      case 'poor':
        recommendations.push(
          'Poor connection quality. Only essential requests are recommended.'
        );
        break;
      case 'fair':
        recommendations.push(
          'Fair connection quality. Large file operations may be slow.'
        );
        break;
      case 'good':
        recommendations.push(
          'Good connection quality. Most operations should work well.'
        );
        break;
      case 'excellent':
        recommendations.push(
          'Excellent connection quality. All operations should be fast.'
        );
        break;
    }
