...
export const useInstructorDataWithToasts = () => {
  ...
  const addAvailabilityWithToast = useCallback(
    async (date: string) => {
      ...
      try {
        ...
        updateToast(loadingId, {
          ...
          message:
            err.userMessage || 'Unable to add availability. Please try again.',
  ...
  const removeAvailabilityWithToast = useCallback(
    async (date: string) => {
      if (!isOnline) {
        warning(
          'You are offline. Changes will be saved when connection is restored.',
  ...
  const updateAttendanceWithToast = useCallback(
    async (studentId: number, attendance: boolean, studentName?: string) => {
      if (!isOnline) {
        warning(
          'You are offline. Attendance changes will be saved when connection is restored.',
          {
            title: 'Offline Mode',
      ...
      const loadingId = loading(
        `Updating attendance for ${studentDisplay}...`,
        {
      ...
      try {
        ...
        updateToast(loadingId, {
          ...
          message:
            err.userMessage || 'Unable to update attendance. Please try again.',
          duration: 0,
          dismissible: true,
          actions: [
            {
              label: 'Retry',
              onClick: () =>
                updateAttendanceWithToast(studentId, attendance, studentName),
              color: 'primary',
              variant: 'contained',
            },
          ],
  ...
  const completeClassWithToast = useCallback(
    async (courseId: number, className?: string) => {
      if (!isOnline) {
        warning(
          'You are offline. Class completion will be processed when connection is restored.',
          {
            title: 'Offline Mode',
      ...
      try {
          ...
          actions: [
            {
              ...
              onClick: () => {
                ...
                info(
                  'Class moved to archive. You can view it in the Archive section.'
                );
              },
              color: 'primary',
            },
        ...
        updateToast(loadingId, {
          ...
          message:
            err.userMessage || 'Unable to complete class. Please try again.',
          duration: 0,
          dismissible: true,
          actions: [
            {
              ...
              ? [
                  {
                    ...
                    onClick: () => {
                      info(
                        'Please ensure all students have their attendance marked before completing the class.'
                      );
                    },
  ...
  const loadDataWithToast = useCallback(async () => {
    if (!isOnline) {
      warning(
        'You are offline. Data will be refreshed when connection is restored.',
        {
          title: 'Offline Mode',
  ...
  const showNetworkStatusToast = useCallback(
  ...
  const showConnectionQualityWarning = useCallback(
    (quality: string) => {
      if (quality === 'poor') {
          ...
          actions: [
            {
              ...
              onClick: () => {
                info(
                  'Try refreshing the page, closing other browser tabs, or moving closer to your WiFi router.'
                );
              },
              color: 'info',
            },
