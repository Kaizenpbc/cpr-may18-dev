...
router.get('/', asyncHandler(async (req: Request<{}, {}, {}, EmailTemplateQueryParams>, res: Response) => {
    console.log('[EMAIL TEMPLATES] GET /api/v1/email-templates - Request received');
    console.log('[EMAIL TEMPLATES] Query params:', req.query);
    console.log('[EMAIL TEMPLATES] User:', req.user);
    
    const { category, active, search } = req.query;
    
    console.log('[EMAIL TEMPLATES] Parsed params:', { category, active, search });
    ...
    console.log('[EMAIL TEMPLATES] Template categories:', templates.map(t => `${t.name}: ${t.category}`));
    
    const response = ApiResponseBuilder.success(templates);
    console.log('[EMAIL TEMPLATES] Sending response with', templates.length, 'templates');
...
router.put('/:id', authorizeRoles('admin'), asyncHandler(async (req: Request<{ id: string }, {}, EmailTemplateRequestBody>, res: Response) => {
    ...
    const updatedTemplate = await EmailTemplateService.update(parseInt(req.params.id), {
        name,
        category: Array.isArray(category) ? category[0] : category, // Take first item if array, otherwise use as-is
        subCategory,
        subject,
...
router.post('/:id/preview', asyncHandler(async (req: Request, res: Response) => {
    ...
    const renderedBody = await EmailTemplateService.renderTemplate(template, variables);
...
router.post('/:id/test', asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!recipientEmail) {
        throw new AppError(400, 'VALIDATION_ERROR', 'Recipient email is required');
    }
...
router.post('/:id/clone', asyncHandler(async (req: Request & { user?: { userId: string } }, res: Response) => {
    const originalTemplate = await EmailTemplateService.getById(parseInt(req.params.id));
    ...
    if (!name) {
        throw new AppError(400, 'VALIDATION_ERROR', 'New template name is required');
    }
    ...
    return res.status(201).json(ApiResponseBuilder.success({
...
router.get('/meta/event-triggers', asyncHandler(async (_req: Request, res: Response) => {
    const eventTriggers = [
...
router.get('/meta/variables', asyncHandler(async (_req: Request, res: Response) => {
