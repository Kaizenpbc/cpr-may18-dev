...
class SessionManager {
  ...
  constructor() {
    this.config = {
      ...
      sessionTimeoutWarning: parseInt(process.env.SESSION_TIMEOUT_WARNING || '300'), // 5 minutes
      ...
      userAgentBindingEnabled: process.env.USER_AGENT_BINDING_ENABLED !== 'false'
    };
  }
   ...
   */
  private determineSecurityLevel(role: string, ipAddress: string): 'standard' | 'high' | 'critical' {
   ...
   */
  private getSessionExpiry(securityLevel: 'standard' | 'high' | 'critical'): number {
  ...
  async createSession(userData: {
    ...
    userAgent: string;
    deviceInfo?: string;
  }): Promise<{ sessionId: string; accessToken: string; refreshToken: string; sessionData: SessionData }> {
    try {
      console.log('🔐 [SESSION] Creating new session for user:', userData.username);

      // Check if user has too many active sessions
      await this.enforceSessionLimits(userData.userId);

      const sessionId = this.generateSessionId();
      const securityLevel = this.determineSecurityLevel(userData.role, userData.ipAddress);
      ...
      console.log(`🔐 [SESSION] Session ${sessionId} created successfully (security: ${securityLevel}, expires: ${sessionExpiry}s)`);
  ...
  async validateSession(
    ...
    try {
      ...
      if (this.config.ipBindingEnabled && sessionData.ipAddress !== ipAddress) {
        console.error(`🚨 [SESSION SECURITY] IP mismatch for session ${sessionId}: expected ${sessionData.ipAddress}, got ${ipAddress}`);
      ...
      if (this.config.userAgentBindingEnabled && sessionData.userAgent !== userAgent) {
        const originalUA = this.extractUserAgentFingerprint(sessionData.userAgent);
        const currentUA = this.extractUserAgentFingerprint(userAgent);
        
        if (originalUA !== currentUA) {
          console.error(`🚨 [SESSION SECURITY] User agent mismatch for session ${sessionId}`);
          await this.invalidateSession(sessionId, 'User agent validation failed');
   ...
   */
  async refreshSession(refreshToken: string, ipAddress: string, userAgent: string): Promise<{
    success: boolean;
    accessToken?: string;
    ...
    try {
      ...
      // Validate session
      const validation = await this.validateSession(decoded.sessionId, ipAddress, userAgent);
      ...
      if (sessionData.refreshToken !== refreshToken) {
        console.error(`🚨 [SESSION SECURITY] Refresh token mismatch for session ${decoded.sessionId}`);
        await this.invalidateSession(decoded.sessionId, 'Refresh token mismatch');
      ...
      const sessionExpiry = this.getSessionExpiry(sessionData.securityLevel);
      await redisManager.setSession(sessionData.sessionId, sessionData, sessionExpiry);

      console.log(`🔐 [SESSION] Session ${sessionData.sessionId} refreshed successfully`);
  ...
  async invalidateSession(sessionId: string, reason?: string): Promise<boolean> {
    try {
      console.log(`🔐 [SESSION] Invalidating session ${sessionId}${reason ? ` (reason: ${reason})` : ''}`);
      
      const result = await redisManager.deleteSession(sessionId);
      
      if (result) {
        console.log(`✅ [SESSION] Session ${sessionId} invalidated successfully`);
        // TODO: Log security event for audit trail
        return true;
      } else {
        console.log(`⚠️ [SESSION] Session ${sessionId} was not found for invalidation`);
        return false;
      }
    } catch (error) {
  ...
  async invalidateAllUserSessions(userId: string, excludeSessionId?: string): Promise<number> {
    try {
      console.log(`🔐 [SESSION] Invalidating all sessions for user ${userId}${excludeSessionId ? ` except ${excludeSessionId}` : ''}`);
      ...
      for (const sessionId of userSessions) {
        ...
        const success = await this.invalidateSession(sessionId, 'User session cleanup');
        if (success) {
          invalidatedCount++;
        }
      }
  ...
  private async enforceSessionLimits(userId: string): Promise<void> {
    try {
      ...
      if (userSessions.length >= this.config.maxSessionsPerUser) {
        console.log(`🔐 [SESSION] User ${userId} has ${userSessions.length} sessions, enforcing limit of ${this.config.maxSessionsPerUser}`);
        ...
        sessionDetails.sort((a, b) => new Date(a.lastAccess).getTime() - new Date(b.lastAccess).getTime());

        // Remove oldest sessions to make room for new one
        const sessionsToRemove = sessionDetails.length - this.config.maxSessionsPerUser + 1;
        
        for (let i = 0; i < sessionsToRemove; i++) {
          await this.invalidateSession(sessionDetails[i].sessionId, 'Session limit enforcement');
        }
      }
    } catch (error) {
  ...
  async getSessionStats(): Promise<{
    ...
    sessionsBySecurityLevel: { standard: number; high: number; critical: number };
  }> {
    try {
      ...
      for (const key of keys) {
        ...
        if (sessionData) {
          ...
          if (parsed.securityLevel) {
            sessionsBySecurityLevel[parsed.securityLevel as keyof typeof sessionsBySecurityLevel]++;
          }
        }
      }
...
export async function refreshUserSession(
  ...
  const result = await sessionManager.refreshSession(refreshToken, ipAddress, userAgent);
...
export async function invalidateAllUserSessions(userId: string, excludeSessionId?: string): Promise<number> {
  return await sessionManager.invalidateAllUserSessions(userId, excludeSessionId);
} 