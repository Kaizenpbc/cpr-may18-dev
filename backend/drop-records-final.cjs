...
async function dropRecords() {
  ...
  try {
    ...
    // 4. Drop payments for completed courses (via invoices)
    if (await tableExists(client, 'payments') && await tableExists(client, 'invoices')) {
    ...
    // 6. Drop notifications for completed courses
    if (await tableExists(client, 'notifications')) {
      console.log('6. Dropping notifications for completed courses...');
      const notificationsResult = await client.query(`
        DELETE FROM notifications 
        WHERE data->>'course_request_id' IN (
          SELECT id::text FROM course_requests WHERE status = 'completed'
        )
      `);
      console.log(`   ✅ Deleted ${notificationsResult.rowCount} notification records\n`);
    } else {
      console.log('6. Skipping notifications (table does not exist)\n');
    }
    
    // 7. Drop activity logs for completed courses
    if (await tableExists(client, 'activity_logs')) {
      console.log('7. Dropping activity logs for completed courses...');
      const logsResult = await client.query(`
        DELETE FROM activity_logs 
        WHERE details LIKE '%course_request_id%' 
        AND details IN (
          SELECT CONCAT('%', id::text, '%') FROM course_requests WHERE status = 'completed'
        )
      `);
      console.log(`   ✅ Deleted ${logsResult.rowCount} activity log records\n`);
    } else {
      console.log('7. Skipping activity_logs (table does not exist)\n');
    }
    
    // 8. Now drop completed courses (after all dependencies are removed)
    if (await tableExists(client, 'course_requests')) {
      console.log('8. Dropping completed courses...');
      ...
      console.log('8. Skipping course_requests (table does not exist)\n');
    }
    
    // 9. Clean up any remaining orphaned records
    if (await tableExists(client, 'course_students')) {
      console.log('9. Cleaning up any remaining orphaned records...');
      ...
      console.log('9. Skipping orphaned cleanup (course_students table does not exist)\n');
    }
